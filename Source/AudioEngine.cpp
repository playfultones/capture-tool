...
int AudioEngine::getCurrentInputChannel() const
{
...
void AudioEngine::setOutputGainTrim(float trimDb)
{
    ...
    DBG("AudioEngine: Output gain trim set to " + juce::String(trimDb, 1) + " dB");
}
...
AudioEngine::LoadResult AudioEngine::loadReferenceSignal(const juce::File& file)
{
    LoadResult result;
    ...
    if (fileSampleRate != 44100 && fileSampleRate != 48000 && fileSampleRate != 96000)
    {
        result.errorMessage = "Unsupported sample rate: " + juce::String(fileSampleRate) 
    ...
    result.durationSeconds = static_cast<double>(numSamples) / static_cast<double>(fileSampleRate);
    
    DBG("AudioEngine: Loaded reference signal: " + file.getFileName() 
        + " (" + juce::String(numSamples) + " samples @ " + juce::String(fileSampleRate) + " Hz)");
...
double AudioEngine::getReferencePlaybackPosition() const
{
    ...
    return static_cast<double>(position) / static_cast<double>(referenceSignalSampleRate);
}

void AudioEngine::setReferencePlaybackLoop(bool enabled)
{
    referencePlaybackLoop.store(enabled, std::memory_order_release);
    DBG("AudioEngine: Reference playback loop " + juce::String(enabled ? "enabled" : "disabled"));
}
...
{
    return captureState.load(std::memory_order_acquire) == CaptureState::RECORDING;
}
...
void AudioEngine::notifyCaptureComplete(bool success, const juce::String& errorMessage, double durationSeconds)
{
    CaptureResult result;
    result.success = success;
    result.errorMessage = errorMessage;
    result.outputFilePath = captureOutputFile.getFullPathName();
    result.durationSeconds = (durationSeconds >= 0.0) ? durationSeconds : getRecordingDuration();
...
bool AudioEngine::openMonitorDevice(const juce::String& deviceName)
{
    ...
    juce::String error = monitorDeviceManager.initialise(
...
juce::String AudioEngine::getMonitorDevice() const
{
...
void AudioEngine::setMonitorChannel(int channelIndex, bool stereo)
{
    ...
    // Re-open the monitor device if it's already open (to apply new channel config)
...
int AudioEngine::getMonitorChannel() const
{
    return monitorChannelIndex;
}

bool AudioEngine::isMonitorStereo() const
{
    return monitorStereo;
}
...
void AudioEngine::setMonitorGain(float gainDb)
{
    ...
    float linearGain = (gainDb <= -60.0f) ? 0.0f : std::pow(10.0f, gainDb / 20.0f);
...
void AudioEngine::setTestToneEnabled(bool enabled)
{
    ...
    DBG("AudioEngine: Test tone " + juce::String(enabled ? "enabled" : "disabled"));
}
...
AudioEngine::RecordingResult AudioEngine::startRecording(const juce::File& outputFile)
{
    ...
    const juce::AudioIODeviceCallbackContext& /*context*/)
{
    ...
    const bool isInCapture = (captureState.load(std::memory_order_acquire) == CaptureState::RECORDING);
    if (isInCapture)
    {
        juce::int64 delayRemaining = capturePlaybackDelayRemaining.load(std::memory_order_acquire);
        if (delayRemaining > 0)
        {
            ...
            capturePlaybackDelayRemaining.store(delayRemaining, std::memory_order_release);
        }
    }
    
    // Generate output: test tone OR reference signal playback (mutually exclusive for simplicity)
    // All output channels are enabled, so selectedOutputChannel maps directly to the array index
    const bool wantTone = testToneEnabled.load(std::memory_order_acquire);
    const bool wantReferencePlayback = referencePlaybackActive.load(std::memory_order_acquire);
    ...
    if (numOutputChannels > 0 && selectedOutputChannel < numOutputChannels)
    {
        ...
        if (outputData != nullptr)
        {
            if (wantReferencePlayback && referenceSignalBuffer.getNumSamples() > 0)
            {
                ...
                int currentPos = referencePlaybackPosition.load(std::memory_order_acquire);
                const bool looping = referencePlaybackLoop.load(std::memory_order_acquire);
                const float outputGain = outputGainLinear.load(std::memory_order_acquire);
                ...
                for (int i = 0; i < numSamples; ++i)
                {
                    ...
                    if (referencePlaybackFadeGain < targetGain)
                        referencePlaybackFadeGain = std::min(referencePlaybackFadeGain + fadeIncrement, targetGain);
                    
                    if (currentPos < refNumSamples)
                    {
                        outputData[i] = refData[currentPos] * referencePlaybackFadeGain * outputGain;
                        ++currentPos;
                    }
                    else if (looping)
                    {
                ...
                if (!looping && currentPos >= refNumSamples)
                {
                    ...
                    if (isInCapture && capturePlaybackStarted.load(std::memory_order_acquire))
                    {
                        int tailSamples = static_cast<int>((static_cast<double>(captureTailMs) / 1000.0) * currentSampleRate);
            ...
            else
            {
                ...
                const float outputGain = outputGainLinear.load(std::memory_order_acquire);
                const double phaseIncrement = (2.0 * juce::MathConstants<double>::pi * testToneFrequency) / currentSampleRate;
                
                for (int i = 0; i < numSamples; ++i)
                {
                    // Smooth fade to prevent clicks
                    if (testToneFadeGain < targetGain)
                        testToneFadeGain = std::min(testToneFadeGain + fadeIncrement, targetGain);
                    else if (testToneFadeGain > targetGain)
                        testToneFadeGain = std::max(testToneFadeGain - fadeIncrement, targetGain);
                    
                    // Generate sine wave sample with output trim applied
                    float sample = static_cast<float>(std::sin(testTonePhase) * testToneAmplitude * testToneFadeGain * outputGain);
                    outputData[i] = sample;
    ...
    // All input channels are enabled, so selectedInputChannel maps directly to the array index
    // Note: No input gain trim - we record the raw input signal.
    ...
    if (isInCapture)
    {
        ...
        if (tailRemaining > 0)
        {
            ...
            if (tailRemaining <= 0)
            {
                ...
                // We can't call stopRecording() directly from audio thread, so signal completion
                captureState.store(CaptureState::DONE, std::memory_order_release);
                
                // Capture the sample count now, before stopping (for duration calculation)
                ...
                juce::MessageManager::callAsync([this, finalSampleCount, sampleRate]()
                {
                    ...
                    // Calculate duration from captured values (before stopRecording cleared them)
                    double durationSeconds = (sampleRate > 0) ? 
                        static_cast<double>(finalSampleCount) / sampleRate : 0.0;
    ...
    // Monitor output routing
    // Route selected source (input or output) to the monitor device via ring buffer
    const MonitorSource currentMonitorSource = monitorSource.load(std::memory_order_acquire);
    auto* monDevice = monitorDeviceManager.getCurrentAudioDevice();
    const bool monitorDeviceReady = (monDevice != nullptr && monDevice->isPlaying());
    ...
    if (currentMonitorSource != MonitorSource::NONE && monitorDeviceReady)
    {
        ...
        {
            const float targetGain = monitorGainLinear.load(std::memory_order_acquire);
            ...
            for (int i = 0; i < samplesToProcess; ++i)
            {
                ...
                const float sample = monitorSourceData[i] * monitorFadeGain * targetGain;
                tempBuffer[i] = sample;
            }
            ...
            if (toWrite > 0)
            {
                ...
                if (size1 > 0)
                {
                    juce::FloatVectorOperations::copy(ringData + start1, tempBuffer, size1);
                }
                
                // Second segment (wrap-around)
                if (size2 > 0)
                {
                    juce::FloatVectorOperations::copy(ringData + start2, tempBuffer + size1, size2);
                }
...
void AudioEngine::audioDeviceAboutToStart(juce::AudioIODevice* device)
{
    if (device != nullptr)
    {
        ...
        DBG("AudioEngine: Device started at " + juce::String(currentSampleRate) + " Hz");
    ...
    const juce::AudioIODeviceCallbackContext& /*context*/)
{
    ...
    if (toRead > 0)
    {
        ...
        const int ch2 = owner.monitorStereo ? 1 : -1;  // Second enabled output channel (if stereo)
        ...
        if (size1 > 0)
        {
            if (ch1 >= 0 && ch1 < numOutputChannels && outputChannelData[ch1] != nullptr)
            {
                juce::FloatVectorOperations::copy(outputChannelData[ch1] + destPos, ringData + start1, size1);
            }
            if (ch2 >= 0 && ch2 < numOutputChannels && outputChannelData[ch2] != nullptr)
            {
                juce::FloatVectorOperations::copy(outputChannelData[ch2] + destPos, ringData + start1, size1);
            }
            destPos += size1;
        }
        ...
        if (size2 > 0)
        {
            if (ch1 >= 0 && ch1 < numOutputChannels && outputChannelData[ch1] != nullptr)
            {
                juce::FloatVectorOperations::copy(outputChannelData[ch1] + destPos, ringData + start2, size2);
            }
            if (ch2 >= 0 && ch2 < numOutputChannels && outputChannelData[ch2] != nullptr)
            {
                juce::FloatVectorOperations::copy(outputChannelData[ch2] + destPos, ringData + start2, size2);
            }
        }
...
void AudioEngine::MonitorDeviceCallback::audioDeviceAboutToStart(juce::AudioIODevice* device)
{
    if (device != nullptr)
    {
